...
@given('a API de {servico} está no ar em "{base_url}"')
def step_set_base_url(context, servico, base_url):
    context.base_url = base_url.rstrip('/')

@given('que existe um cliente com id {ident:d}')
def step_cliente_exists(context, ident):
...
def step_post_with_json(context, path):
    ...
    context.response = requests.post(url, json=payload, auth=BASIC_AUTH)

@when('eu envio um PUT para "{path}" com JSON')
def step_put_with_json(context, path):
    url = f"{context.base_url}{path}"
    payload = json.loads(context.text)
    context.response = requests.put(url, json=payload, auth=BASIC_AUTH)

@when('eu envio um DELETE para "{path}" com JSON')
def step_delete_with_json(context, path):
    url = f"{context.base_url}{path}"
    payload = json.loads(context.text)
    context.response = requests.delete(url, json=payload, auth=BASIC_AUTH)
...
def step_body_contains(context, texto):
    ...
    assert texto in body, f'Esperava "{texto}" em:\n{body}'

@then('a resposta JSON deve ser uma lista')
def step_response_json_list(context):
    data = context.response.json()
    assert isinstance(data, list), f"Esperava lista, recebeu {type(data)}:\n{data}"